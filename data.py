...
DATA_PATH = '/mnt/sdd/data/'

def prepare_tinystories():
    if os.path.isfile('/mnt/sdd/data/tinystories-train-tokenized.npy'):
        return
    ...
    np.save('/mnt/sdd/data/tinystories-train-tokenized.npy', train)
    test = np.array([token_id for line in ts['validation']['token_ids'] for token_id in line], dtype='uint16')
    np.save('/mnt/sdd/data/tinystories-test-tokenized.npy', test)
